...
for(gene_i in 1:nrow(risk_gene)){
  ...
  if(var(y) == 0){
    cat("chrid:", chrid, 
        "  gene:", gene_i, "\n", 
        file = "/gpfs/gibbs/pi/zhao/xz527/knockoff_anno/real_data/GTEx/result_500k_enhancer/log.txt", append = TRUE)
    next
  }
  ...
  clusters <- cutree(hc, h = 0.4)  # e.g., cluster SNPs with r² > 0.5
  ...
  annot = annot[, ..cats]
  length(intersect(colnames(X_repre), annot$SNP))
  index <- match(colnames(X_repre), annot$SNP)
  annot <- annot[index, ]
  annot0 <- annot
  annot <- annot[, -1]
