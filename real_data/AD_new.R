...
for(M in c(1,3,5)){
  for(seed in c(100, 10000)){
    ...
    sum_s <- sum_s[, c('SNP', 'chr', 'pos', 'A1', 'A2', 'z', 'p', 'N')]
    colnames(sum_s) <- c("SNP", "Chromsome", "Position", "EffectAllele", "NonEffectAllele", "Z", "Pval", "Neff")
    sum_s[[study1]] <- sum_s$Pval
    print(length(unique(sum_s$SNP)) == nrow(sum_s))
    
    for(study in study2){
      
      path = paste0("/gpfs/gibbs/pi/zhao/xz527/knockoff_anno/real_data/GK_anno/AD_new/bychr/", 
                    study, "_chr", chrid, ".txt")
      temp <- read.table(path, header = TRUE)
      temp = temp[, c('SNP', 'chr', 'pos', 'A1', 'A2', 'z', 'p', 'N')]
      colnames(temp) <- c("SNP", "Chromsome", "Position", "EffectAllele", "NonEffectAllele", "Z", "Pval", "Neff")
      message(study, ": unique SNP? ", length(unique(temp$SNP)) == nrow(temp))
      index <- match(sum_s$SNP, temp$SNP)
      print(table(is.na(index)))
      print(table(temp$SNP[index] == sum_s$SNP))
      message(study, ": matched = ", sum(!is.na(index)), "/", length(index))
      sum_s[[study]] <- temp$Pval[index]
    }
    ...
    sums_chr <- sum_s[order(sum_s$Position),]
    ...
    # risk_id <- which(sums_chr$GWAS1 < 5 * 10^-8)
    ...
    if(!any(is.na(risk_region))){
      ...
      for(i in 1:nrow(risk_region)){
        ...
        trait_region$Z[is.infinite(trait_region$Z) & trait_region$Z > 0] <- 1e2
        trait_region$Z[is.infinite(trait_region$Z) & trait_region$Z < 0] <- -1e2
        ...
        trait_region$Z[inv] <- -trait_region$Z[inv]
